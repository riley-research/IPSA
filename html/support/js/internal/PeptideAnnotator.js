...
myApp.controller(
  ...
  function (
    ...
    $scope.filteredAnnotatedResults;
    ...
    $scope.validateSequence = function () {
      ...
      for (var i = 0; i < $scope.peptide.sequence.length; i++) {
        ...
        if (!regex.exec(character)) {
          ...
          $scope.peptide.sequence = $scope.peptide.sequence.replace(
            character,
            ""
          );
          alert(
            character +
              " is not a valid amino acid. Only the 20 canonical amino acids are supported."
          );
        }
      }
    };
